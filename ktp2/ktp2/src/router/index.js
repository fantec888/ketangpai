...
router.beforeEach((to, from, next) => {
  if (to.meta.requireAuth) {
    ...
    if (user != undefined) {
      store.commit(mt.IS_LOGIN, user.account)
      store.commit(mt.CHANGE_UID, user.uid)
      store.commit(mt.USER_AVATAR, user.avatar)
      store.commit(mt.PERMISSION, user.permission)
      store.commit(mt.CHANGE_NAME, user.name)
